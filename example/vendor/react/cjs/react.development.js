...
function contextDidUpdate(context, newValue) {
  context._currentValue = context._currentValue2 = newValue;
}

function setContext(context, newValue, userCallback) {
  var oldValue = context._globalValue;
  ...
  if (userCallback !== null && userCallback !== undefined) {
    ...
    var numRootsThatNeedUpdate = 0;
    var _root = ReactRootList.first;
    if (_root !== null) {
      do {
        numRootsThatNeedUpdate += 1;
        _root = _root.nextGlobalRoot;
      } while (_root !== null);
      wrappedCallback = function (committedValue) {
        numRootsThatNeedUpdate -= 1;
...
function createContext(defaultValue, calculateChangedBitsFn) {
  if (calculateChangedBitsFn === undefined) {
    calculateChangedBitsFn = null;
  } else {
    {
      !(calculateChangedBitsFn === null || typeof calculateChangedBitsFn === 'function') ? warningWithoutStack$1(false, 'createContext: Expected the optional second argument to be a ' + 'function. Instead received: %s', calculateChangedBitsFn) : void 0;
    }
  }

  var context = {
    $$typeof: REACT_CONTEXT_TYPE,
    _calculateChangedBits: calculateChangedBitsFn,
