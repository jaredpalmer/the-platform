...
function updateHostRoot(current$$1, workInProgress, renderExpirationTime) {
  ...
  var contextUpdate = nextState.firstContextUpdate;
  if (contextUpdate !== null) {
    propagateRootContextChanges(workInProgress, contextUpdate, renderExpirationTime);
  }
...
function calculateChangedBits(context, oldValue, newValue) {
  ...
  if (oldValue === newValue && (oldValue !== 0 || 1 / oldValue === 1 / newValue) || oldValue !== oldValue && newValue !== newValue // eslint-disable-line no-self-compare
    ...
    var _changedBits = typeof context._calculateChangedBits === 'function' ? context._calculateChangedBits(oldValue, newValue) : maxSigned31BitInt;

    {
      !((_changedBits & maxSigned31BitInt) === _changedBits) ? warning$1(false, 'calculateChangedBits: Expected the return value to be a ' + '31-bit integer. Instead received: %s', _changedBits) : void 0;
    }
    return _changedBits | 0;
  }
}

function setRootContext(fiber, context, oldValue, newValue, callback) {
  // Schedule an update on the root.
  var currentTime = requestCurrentTime();
  var expirationTime = computeExpirationForFiber(currentTime, fiber);
  var update = createUpdate(expirationTime);
  var changedBits = calculateChangedBits(context, oldValue, newValue);
  update.payload = function (state) {
    var contextUpdate = {
      context: context,
      valueToResetTo: oldValue,
      value: newValue,
    ...
    var firstContextUpdate = state.firstContextUpdate;
    var lastContextUpdate = state.lastContextUpdate;
    ...
    return {
      firstContextUpdate: firstContextUpdate,
...
function propagateRootContextChanges(workInProgress, firstContextUpdate, renderExpirationTime) {
  var contextUpdate = firstContextUpdate;
  do {
    var _context = contextUpdate.context;
    var _value = contextUpdate.value;
    var _changedBits2 = contextUpdate.changedBits;
    if (isPrimaryRenderer) {
      _context._currentValue = _value;
      _context._changedBits = _changedBits2;
    } else {
      _context._currentValue2 = _value;
      _context._changedBits2 = _changedBits2;
    }
    propagateContextChange(workInProgress, _context, _changedBits2, renderExpirationTime);
    contextUpdate = contextUpdate.next;
  } while (contextUpdate !== null);
}

function popRootContexts(workInProgress) {
  var state = workInProgress.memoizedState;
  var firstContextUpdate = state.firstContextUpdate;
  if (firstContextUpdate !== null) {
    var contextUpdate = firstContextUpdate;
    do {
      var _context2 = contextUpdate.context;
      var _valueToResetTo = contextUpdate.valueToResetTo;
      if (isPrimaryRenderer) {
        _context2._currentValue = _valueToResetTo;
        _context2._changedBits = 0;
      } else {
        _context2._currentValue2 = _valueToResetTo;
        _context2._changedBits2 = 0;
      }
      contextUpdate = contextUpdate.next;
    } while (contextUpdate !== null);
  }
  state.firstContextUpdate = state.lastContextUpdate = null;
}
...
function propagateContextChange(workInProgress, context, changedBits, renderExpirationTime) {
  ...
  while (fiber !== null) {
    ...
    if (dependency !== null) {
      ...
      var providerContext = fiber.type._context;
...
function prepareToReadContext(workInProgress, renderExpirationTime) {
  ...
  if (firstContextDependency !== null) {
    ...
    do {
      var _context3 = dependency.context;
      var _changedBits3 = isPrimaryRenderer ? _context3._changedBits : _context3._changedBits2;
      if (_changedBits3 !== 0) {
        ...
        // bits than this one.
        propagateContextChange(workInProgress, _context3, _changedBits3, renderExpirationTime);
        if ((_changedBits3 & dependency.observedBits) !== 0) {
...
var ReactRootList$1 = ReactSharedInternals.ReactRootList;

// TODO: This should be lifted into the renderer.

function createFiberRoot(containerInfo, isAsync, hydrate) {
  var lastGlobalRoot = ReactRootList$1.last;
  ...
  var root = {
    ...
    setContext: setRootContext.bind(null, uninitializedFiber)
  };
  ...
  if (lastGlobalRoot === null) {
    ReactRootList$1.first = root;
  } else {
    lastGlobalRoot.nextGlobalRoot = root;
  }
  ReactRootList$1.last = root;

  return root;
}

var ReactRootList = ReactSharedInternals.ReactRootList;
...
function unmountRootFromGlobalList(root) {
  ...
  var previous = root.previousGlobalRoot;
  var next = root.nextGlobalRoot;
  if (previous !== null) {
...
function updateContainerAtExpirationTime(element, container, parentComponent, expirationTime, callback) {
  ...
  return scheduleRootUpdate(current$$1, element, expirationTime, wrappedCallback);
}
